...
const ProductListingScreenSex = ({navigation, route}) => {
  // const {sex} = props;
  ...
  const [savedList, setSavedList] = useState(0)
   ...
   useEffect(() => {
    const unsubscribe = navigation.addListener('focus', async () => {
      const iSex = { sex :route.params.sex }
      let response = await fetch('http://80.249.151.43/getProductsBySex', {method:"POST", headers: {'Content-Type': 'application/json;charset=utf-8'}, body: JSON.stringify(iSex)} )
      const data = await response.json()
      setProductList(data)
      setTitle(route.params.sex)
      const list = JSON.parse(await AsyncStorage.getItem('FavItems'))
      setSavedList(list)
    })
    return unsubscribe;
...
ProductListingScreenSex.propTypes = {};

ProductListingScreenSex.defaultProps = {};
export default ProductListingScreenSex;